...
apiclient.setupMockSend = function(calls) {
  ...
  const formatMockHeaders = (contentType, headers) => {
    ...
    // The XMLHttpRequest specification (and RFC 9112 (HTTP/1.1)) mandates that
    // getAllResponseHeaders() separates headers with CRLF ("\r\n").
    return lines.join('\r\n');
  };
...
/**
 * A function called when quota status results are received from the server.
 * Takes the header value as an argument. Null if not set.
 * @private {?function(string)}
 */
