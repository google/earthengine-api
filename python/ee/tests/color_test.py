...
class ColorTest(apitestcase.ApiTestCase):
  ...
  def test_constructors(self):
    ...
    self.assertEqual(
        ee.ApiFunction('Color'), ee.Color('red').func
    )
    self.assertEqual(
        {'input': 'red'}, ee.Color('red').args
    )

    self.assertEqual(
        ee.ApiFunction('Color'), ee.Color([1, 0, 0]).func
    )
    self.assertEqual(
        {'input': [1, 0, 0]}, ee.Color([1, 0, 0]).args
    )
  ...
  def test_static_methods(self):
    ...
    self.assertEqual(
        ee.ApiFunction('Color.gray'), ee.Color.gray(0.5).func
    )
    self.assertEqual(
        ee.ApiFunction('Color.mix'), ee.Color.mix(ee.Color('red'), ee.Color('blue')).func
    )
  ...
  def test_instance_methods(self):
    ...
    self.assertEqual(ee.ApiFunction('Color.toHexString'), color.toHexString().func)
