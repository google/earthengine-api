 ...
 * @constructor
 */
exporter.App = function(channelToken, clientId) {
  // The Google Map.
...
exporter.App.prototype.initDatePicker = function() {
 ...
 */
exporter.App.prototype.getYearParam = function() {
...
exporter.App.prototype.initRegionPicker = function() {
 ...
 */
exporter.App.prototype.getPolygonCoordinates = function() {
 ...
 */
exporter.App.prototype.setDrawingModeEnabled = function(enabled) {
 ...
 */
exporter.App.prototype.getDrawingModeEnabled = function() {
  return $('.region').hasClass('drawing');
};


/** Clears the current polygon from the map and enables drawing. */
exporter.App.prototype.clearPolygon = function() {
 ...
 */
exporter.App.prototype.handleNewPolygon = function(opt_overlay) {
 ...
 */
exporter.App.prototype.setAlert = function(name, cls, line1, opt_line2) {
  var alert;
 ...
 */
exporter.App.prototype.removeAlert = function(name) {
 ...
 */
exporter.App.prototype.findAlert = function(name) {
...
exporter.App.prototype.refreshImage = function() {
  var name = 'lights';
 ...
 */
exporter.App.prototype.setLayer = function(name, mapIdPath, addOnTop) {
  this.removeLayer(name);
  ...
  var onError = (function(error) {
    ...
    this.setAlert(
        name, 'danger', 'Map layer "' + name + '" failed to load.', error);
  }).bind(this);
  ...
  this.layerRequests[name] = exporter.App.handleRequest(
      $.get(mapIdPath), onDone, onError);
};
 ...
 */
exporter.App.prototype.removeLayer = function(name) {
...
exporter.App.prototype.initExport = function() {
  this.channel.open({
    'onmessage': (function(message) {
      ...
      if (data.error) {
        ...
        var isMultipleFiles = data.link.indexOf('open?id=') == -1;
        var line2;
        if (isMultipleFiles) {
          line2 = $('<span/>')
              .append(link)
              .append('<span class="multifile">(multiple files)</span>');
        } else {
          line2 = link;
        }
...
/**
 * Returns the filename entered in the input box or a generated default filename
 * if none.
 * @return {string} The name to give the exported file in the user's Drive.
 */
exporter.App.prototype.getFilename = function() {
  var userProvidedFilename = $('.filename').val();
  if (userProvidedFilename) {
    return userProvidedFilename;
  } else {
    return 'Lights_Export_' + (new Date()).toISOString().replace(/[^0-9]/g, '');
  }
};
 ...
 */
exporter.App.prototype.exportImage = function() {
 ...
 */
exporter.App.createMap = function(el) {
  var mapOptions = {
 ...
 *     the given map.
 */
exporter.App.createDrawingManager = function(map) {
 ...
 *     can be registered.
 */
exporter.App.handleRequest = function(request, onDone, onError) {
...
exporter.App.DEFAULT_CENTER = {lng: -91.6, lat: 38.5};
